...
**The correct quiz answer used to keep landing in the first option.**
That bias was not reliably fixed by prompt wording alone, so the Skill now ships a concrete quiz asset seed instead. The first multiple-choice lesson copies `QUIZ-ASSET.js` into `<teaching-workspace>/assets/quiz.js`; the browser component shuffles semantic options at render time and tracks correctness on the option itself rather than by ordinal position. If the workspace asset cannot be created or loaded, `teach` falls back to free-response retrieval instead of emitting a static A/B/C quiz. Existing teaching workspaces keep ownership of their copied `assets/quiz.js`; updating the Skill does not overwrite a locally modified component.
