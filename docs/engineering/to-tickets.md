...
Do not assume the installed `gh` exposes the newest sub-issue flags. The tracker adapter capability-probes the local CLI first. When native `--parent` / `--add-sub-issue` conveniences exist, use them; otherwise keep the same native GitHub sub-issue relationship through `gh api` and the REST `sub_issues` endpoint. Only when the GitHub sub-issue API itself is unavailable should the adapter fall back to a task-list/body representation.

**"Blocked by" was written into the issue body instead of a real blocking link.**
Likewise, GitHub dependency semantics are capability-driven rather than version-assumed. A newer `gh` may expose `--blocked-by`, `--add-blocked-by` and `blockedBy`; an older packaged CLI may expose none of them while still supporting authenticated `gh api`. In that case the adapter uses the REST `dependencies/blocked_by` endpoint so the relationship remains native and UI-visible. Body text is only the last compatibility fallback when the dependency API itself is unavailable.
