...
No. They are already specification-ready — `to-tickets` applies the `ready-for-agent` role as it publishes. AFK execution should still query the tracker frontier, so a ticket with unresolved blockers is not pickable merely because it carries that role. `triage` is the on-ramp for work that arrives from outside; the spec flow is the lane for work you originate. They meet at `ready-for-agent`, not before.
...
**Five states aren't enough — what about blocked, deferred, or implemented?**
The state machine now keeps those concerns separate instead of adding more state labels. `ready-for-agent` means the work is sufficiently specified; whether it can be picked up **now** is decided by the configured tracker's frontier. A fully specified item waiting on another issue keeps `ready-for-agent` plus an explicit blocker edge, and AFK pickup must query state + frontier rather than the label alone. Future work gated by a non-ticket external condition stays `needs-triage` with a specific trigger note until that condition becomes actionable or can be represented as a blocker. Completed ordinary implementation tickets are closed/resolved by `implement` only after their acceptance criteria and required review are established, so "implemented" is not another triage state.

**How is this different from `/diagnosing-bugs`?**
The verification step here is deliberately shallow — enough to answer "is this real, and roughly where does it live", not to find a root cause. When a bug will not reproduce quickly from the reporter's steps, the default triage move is `needs-info`, not an unbounded diagnosis. If the maintainer explicitly wants to chase the root cause now, triage conditionally loads `diagnosing-bugs`; the diagnosis can inform the recommendation, but triage still owns the issue state and labels.

**Can I point it at my whole backlog and let it run?**
Yes, but the cheap "show what needs attention" listing remains selection-only. In a bulk pass the canonical Skill must process each selected item through the full specific-item flow — body, comments/history, labels and relevant code or diff — before it changes that item's state. A bulk run that reuses list output as evidence is a broken run, even if the list happened to contain enough text for some items.
