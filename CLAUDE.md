...
For maintainer-only Claude Code development, `scripts/link-skills.sh` links this checkout into `~/.claude/skills`. It intentionally does not write `~/.agents/skills`: in the Akira environment that path is the machine-level registry managed from remote GitHub checkouts by the Akira installer, not a harness-specific local-development destination.
