...
description: Writing documents for agents. Use when creating or editing skills, project Agent instructions such as AGENTS.md or executor-specific compatibility files such as CLAUDE.md, or other documents intended primarily for Agent consumption.
---

Reference for writing any document an agent consumes — a skill, canonical project Agent instructions, an executor-specific compatibility file, or a doc reached by a pointer. The packaging differs; the writing does not: the same levers make each one predictable — the agent taking the same _process_ every run, not producing the same output.
